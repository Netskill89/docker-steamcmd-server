...
if [ "${ENABLE_BEPINEX}" == "true" ]; then
    ...
    LAT_V="$(wget -qO- https://github.com/BepInEx/BepInEx/releases/latest/ | jq -r '.latest.version_number')"
    if [ -z "${LAT_V}" ] && [ -z "${CUR_V}" ]; then
    ...
    echo
    echo "---https://github.com/BepInEx/BepInEx/releases/latest---"
    echo
    if [ -z "${CUR_V}" ]; then
        echo "---BepInEx for Valheim not found, downloading and installing v$LAT_V...---"
        cd ${SERVER_DIR}
        if wget -q -nc --show-progress --progress=bar:force:noscroll -O ${SERVER_DIR}/BepInEx.zip --user-agent=Mozilla --content-disposition -E -c "https://github.com/BepInEx/BepInEx/releases/tag/$LAT_V/" ; then
